...
const ezRegex = /ez/gi
const finalTRegex = /t\b(?!\s+(?:Fleet|Constant)\b)/gi
...
function translateJostophe(sourceId, resultId) {
    ...
    translatedText = translatedText.replace(éRegex,"ai").replace(aiRegex,"é")
    ...
    translatedText = translatedText.replace(ezRegex,"é")
    translatedText = translatedText.replace(quRegex,"k")
    ...
    translatedText = translatedText.replace(mmRegex, "m")
